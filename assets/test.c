...
void create_philosophers(int n, int time_to_die, int time_to_eat, int time_to_sleep) {
    sem_t *forks = sem_open(SEM_FORK_NAME, O_CREAT, 0644, n); // Semaphore for forks
    sem_t *sem_print = sem_open("/sem_print", O_CREAT, 0644, 1); // Semaphore for safe printing
...
int main(int argc, char **argv) {
    if (argc < 5) {
        printf("Usage: %s <num_of_philosophers> <time_to_die> <time_to_eat> <time_to_sleep>\n", argv[0]);
        return 1;
    }
    ...
    create_philosophers(num_of_philosophers, time_to_die, time_to_eat, time_to_sleep);
